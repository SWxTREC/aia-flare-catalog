...
def plot_flare_nogoes(flare,aia_times,aia_data,window):
    lams = ['193','171','304','1600','131','94']
    colors = ['tab:blue','tab:orange','tab:green','tab:red','tab:purple','tab:brown']
    ...
    for j in range(len(lams)):
        ...
        ax[j].legend()
        ax[j].grid()
    ax[0].set_title('SHARP '+str(flare['SHARP'])+': predicted '+str(flare['ert_pred_CMX'])+str(flare['ert_pred_intensity'])[:4]+ ', mean start time:'+str(flare['mean_start_time']))
